...
def train(args):
    ...
    train_indexes_hdf5_path = os.path.join(workspace, 'hdf5s', 'indexes', 'train.h5')

    # eval_bal_indexes_hdf5_path = os.path.join(workspace, 
    #     'hdf5s', 'indexes', 'train.h5')
    ...
    checkpoints_dir = os.path.join(workspace, 'checkpoints_1', filename)
    ...
    eval_test_sampler = EvaluateSampler(
        indexes_hdf5_path=eval_test_indexes_hdf5_path, batch_size=1024)
    ...
    for batch_data_dict in train_loader:
        ...
        for batch_data_eval_dict in eval_test_loader:
            # print(batch_data_eval_dict)
            batch_output_eval_dict = model(torch.from_numpy(batch_data_eval_dict['waveform']), None)
            """{'clipwise_output': (batch_size, classes_num), ...}"""

            # print(type(batch_data_eval_dict['target']))
            # print(type(batch_data_dict['target']))
            
            batch_target_eval_dict = {'target': torch.from_numpy(batch_data_eval_dict['target'])}
            """{'target': (batch_size, classes_num)}"""
        
        # for key in batch_data_dict.keys():
        #     batch_data_dict[key] = move_data_to_device(batch_data_dict[key], device)
        
        eval_loss = loss_func(batch_output_eval_dict, batch_target_eval_dict)

        # print(type(eval_loss.item()))
        # print(loss)
